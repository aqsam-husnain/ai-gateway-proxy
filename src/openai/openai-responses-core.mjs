...
class StreamState {
  constructor() {
    this.states = new Map(); // Use Map to store state for different requests
  }
...
// Create global streaming state manager
...
function generateResponseCompleted(requestId, usage) {
  ...
  return {
    ...
    response: {
      ...
      safety_identifier: `user-${uuidv4().replace(/-/g, '')}`, // Random value
      ...
      truncation: "disabled",
      usage: usage || {
        input_tokens: Math.floor(Math.random() * 100) + 20, // Random value
        input_tokens_details: {
          cached_tokens: Math.floor(Math.random() * 50) // Random value
        },
        output_tokens: state.fullText.split('').length,
        output_tokens_details: {
          reasoning_tokens: 0
        },
        total_tokens: Math.floor(Math.random() * 100) + 20 + state.fullText.split('').length // Random value + text length
      },
      user: null
    }
  };
...
// Export streaming state manager for external use
