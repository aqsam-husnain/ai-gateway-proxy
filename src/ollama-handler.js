/**
 * Ollama API Handler
 * Handles Ollama-specific endpoints and converts between backend protocols
 */
...
export function removeModelPrefix(modelName) {
    ...
    // Remove any prefix pattern like [Claude], [Gemini CLI], etc.
...
export function getProviderByModelName(modelName, providerPoolManager, defaultProvider) {
    ...
    if (lowerModelName.includes('claude') || lowerModelName.includes('sonnet') || lowerModelName.includes('opus') || lowerModelName.includes('haiku')) {
        ...
        for (const [providerType, providers] of Object.entries(providerPoolManager.providerPools)) {
            if (providerType.includes('claude')) {
...
/**
 * Normalizes Ollama path and checks if it's an Ollama endpoint
...
/**
 * Handles all Ollama-related path normalization and endpoint routing
...
/**
 * Handles Ollama endpoint routing (before auth check)
 * @param {string} method - HTTP method
 * @param {string} path - Request path
 * @param {Object} req - Request object
 * @param {Object} res - Response object
...
/**
 * Handles Ollama endpoint routing (after auth check)
 * @param {string} method - HTTP method
 * @param {string} path - Request path
 * @param {Object} req - Request object
 * @param {Object} res - Response object
...
/**
 * Handles Ollama /api/tags endpoint (list models)
 */
...
/**
 * Handles Ollama /api/show endpoint (display model information)
 */
...
/**
 * Handles Ollama /api/version endpoint
 */
...
/**
 * Handles Ollama /api/chat endpoint
 */
...
/**
 * Handles Ollama /api/generate endpoint
 */
