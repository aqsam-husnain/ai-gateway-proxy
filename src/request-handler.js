...
export function createRequestHandler(config, providerPoolManager) {
    return async function requestHandler(req, res) {
        ...
        if (path.startsWith('/static/') || path === '/' || path === '/favicon.ico' || path === '/index.html' || path.startsWith('/app/') || path === '/login.html' || path === '/openapi.json' || path === '/dashboard.html' || path === '/dashboard') {
        ...
        // Supports unhealthRatioThreshold to control unhealthy ratio threshold, returns summaryHealthy: false when unhealthyRatio exceeds threshold
        ...
        if (modelProviderHeader) {
            ...
            currentConfig.explicitProviderSet = true; // Track explicit provider selection
