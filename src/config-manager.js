...
export async function initializeConfig(args = process.argv.slice(2), configFilePath = 'configs/config.json') {
    ...
    try {
        ...
        currentConfig = {
            ...
            PROVIDER_POOLS_FILE_PATH: null, // Provider pools configuration file path
            MAX_ERROR_COUNT: 3, // Maximum error count for provider
            HEALTH_CHECK_INTERVAL_HOURS: 1, // Hours before retrying unhealthy providers
            // Auto health check configuration
            QUICK_RETRY_INTERVAL_SECONDS: 10, // Seconds between quick retries for non-429 errors
            QUICK_RETRY_MAX_COUNT: 3, // Maximum quick retry attempts
            RATE_LIMIT_CHECK_INTERVAL_HOURS: 3, // Hours before retrying rate-limited (429) providers
            STANDARD_CHECK_INTERVAL_HOURS: 3, // Hours for standard health check interval (after quick retries exhausted)
            AUTO_HEALTH_CHECK_ENABLED: true, // Enable/disable auto health checks
            providerFallbackChain: {} // Cross-type fallback chain configuration
        };
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        } else if (args[i] === '--health-check-interval-hours') {
            if (i + 1 < args.length) {
                currentConfig.HEALTH_CHECK_INTERVAL_HOURS = parseFloat(args[i + 1]);
                i++;
            } else {
                console.warn(`[Config Warning] --health-check-interval-hours flag requires a value.`);
            }
        } else if (args[i] === '--quick-retry-interval-seconds') {
            if (i + 1 < args.length) {
                currentConfig.QUICK_RETRY_INTERVAL_SECONDS = parseInt(args[i + 1], 10);
                i++;
            } else {
                console.warn(`[Config Warning] --quick-retry-interval-seconds flag requires a value.`);
            }
        } else if (args[i] === '--quick-retry-max-count') {
            if (i + 1 < args.length) {
                currentConfig.QUICK_RETRY_MAX_COUNT = parseInt(args[i + 1], 10);
                i++;
            } else {
                console.warn(`[Config Warning] --quick-retry-max-count flag requires a value.`);
            }
        } else if (args[i] === '--rate-limit-check-interval-hours') {
            if (i + 1 < args.length) {
                currentConfig.RATE_LIMIT_CHECK_INTERVAL_HOURS = parseFloat(args[i + 1]);
                i++;
            } else {
                console.warn(`[Config Warning] --rate-limit-check-interval-hours flag requires a value.`);
            }
        } else if (args[i] === '--standard-check-interval-hours') {
            if (i + 1 < args.length) {
                currentConfig.STANDARD_CHECK_INTERVAL_HOURS = parseFloat(args[i + 1]);
                i++;
            } else {
                console.warn(`[Config Warning] --standard-check-interval-hours flag requires a value.`);
            }
        } else if (args[i] === '--auto-health-check-enabled') {
            if (i + 1 < args.length) {
                currentConfig.AUTO_HEALTH_CHECK_ENABLED = args[i + 1].toLowerCase() === 'true';
                i++;
            } else {
                console.warn(`[Config Warning] --auto-health-check-enabled flag requires a value.`);
            }
        }
    }
    ...
    // Load provider pools configuration
